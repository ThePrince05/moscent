...
import { FiDollarSign, FiShoppingCart, FiUsers, FiPackage, FiBox, FiAlertTriangle, FiPlusCircle } from 'react-icons/fi';
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              <div className={`flex flex-col items-center justify-center py-4 px-6 border border-gray-300 rounded-md
                ...
                <FiDollarSign size={32} className={`mb-2 group-hover:text-accent-red transition-colors`} />
                      ...
                      <tr key={order.id} className="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td className={`px-5 py-5 border-b text-sm border-[#D3D3D3]`}>
                          <span className={`relative inline-block px-3 py-1 font-semibold leading-tight
                            ${order.status === 'Processing' ? 'text-orange-900' : order.status === 'Shipped' ? 'text-blue-900' : 'text-green-900'}`}>
                            <span aria-hidden="true" className={`absolute inset-0 opacity-50 rounded-full
                              ${order.status === 'Processing' ? 'bg-orange-200' : order.status === 'Shipped' ? 'bg-blue-200' : 'bg-green-200'}`}></span>
